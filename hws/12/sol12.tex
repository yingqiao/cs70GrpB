...
&\quad\text{group summations} \\ 
&= \sum\nolimits_{i=1}^n (\exx[X_i^2] - \exx[X_i]^2) &\quad\text{$X_i$ and $X_j$ are pair-wise independent, use lemma}
\end{align*}
}}
\fi
...
\ifsolutions{ \answer {
First we come up with two random variables $X$ and $Y$ such that $Var[X+Y] \neq Var[X] + Var[Y]$. Let us let $X$ count the number of heads after 1 flip of a fair coin and let $Y$ count the number of tails after the same 1 flip of that coin. Then $X+Y = 1$ always, and therefore the Variance is 0. The Variance of a constant number is 0. However, 
\begin{align*}
Var[X] &= \exx[X^2] - \exx[X]^2 \\
&= Pr[X=1]\cdot1^2 + Pr[X=0]\cdot0^2 - (Pr[X=1]\cdot1 + Pr[X=0]\cdot0)^2 \\ 
&= 1/2\cdot1 - (1/2\cdot1)^2 \\
&= 1/2 - 1/4 = 1/4
\end{align*} 
...
\binom{2}{0}(1/3)^0(1-(1/3))^{2-0} + \binom{2}{1}(1/3)^1(1-(1/3))^{2-1} + \binom{2}{2}(1/3)^2(1-(1/3))^{2-2} \\ 
&= \binom{2}{0}(1/3)^0(2/3)^{2} + \binom{2}{1}(1/3)^1(2/3)^1 + \binom{2}{2}(1/3)^2(2/3)^0 \\ 
&= \binom{2}{0}(4/9) + \binom{2}{1}(2/9) + \binom{2}{2}(1/9) \\
&=  1(4/9) + 2(2/9) + 1(1/9) = 4/9 + 4/9 +1/9 \\ 
&= 1
\end{align*}
  }} 
  \fi
...
\begin{align*}
(a+b)^n &=  \sum\nolimits_{k=0}^n \binom{n}{k} a^{n-k}b^k &\text{By the binomial theorem} \\ 
&=  \sum\nolimits_{k=0}^n \binom{n}{k} (1-p)^{n-k}p^k &\text{plugging in the values of $a$ and $b$} \\
&= \sum\nolimits_{k=0}^n Pr[X=k] &\text{using our answer to part f} \\
&= (a+b)^n = (1-p+p)^n = 1^n \\
&= 1
\end{align*}
...
\item Use the linearity of expectation to calculate $\mathbb{E}[X]$.
 ...
 \ifsolutions{ \answer {
By the linearity of expectation, if $X = \sum\nolimits_{i=1}^nX_i$, then $\exx[X] = \sum\nolimits_{i=1}^n\exx[X_i]$. So now we must compute the expectation of each $X_i$. We notice that each $X_i$ is distributed identically, therefore it is sufficient to compute the expectation of only one $X_i$ and the rest will be identical. 

Each $X_i$ is 1 (flip heads on $i^{\text{th}}$ flip) with probability $p$ and 0  (flip tails on $i^{\text{th}}$ flip) with probability $1-p$. Therefore, $\exx[X_i] = 1\cdot p + 0 \cdot(1-p) = p$ by definition of expectation. And $\exx[X] = \sum\nolimits_{i=1}^n\exx[X_i] = \sum\nolimits_{i=1}^n p = np$. If $X \sim B(n,p)$ then, $\exx[X] = np$.
  }} 
  \fi 
\qpart
...
%Using the notation in the extremely helpful hint, $E[T]=E[T|A]\Pr[A]+E[T|S]\Pr[S]+E[T|D]\Pr[D]=\frac{1}{3}(E[T|A]+E[T|S]+E[T|D])$. Note that $E[T|A]=E[T]+2$ by the memorylessness of the situation. Similarly, $E[T|S]=E[T]+5$. Lastly, $E[T|D]=0$ because Mr. Bond escapes immediately. So we have that $E[T]=\frac{1}{3}(E[T]+2+E[T]+5+0)$. Solving this for $E[T]$, we get that $E[T]=\fbox{\ensuremath{\mathbf{007}}}$.
}}
