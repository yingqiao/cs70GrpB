...
\ifsolutions{ \answer{
We want to define $X_1, X_2, X_3, X_4$ on $\Omega = \{-1, 1\}$ satisfying the independece requirement given in the problem. We recall the coin toss experiment and define $H$ being $1$ while $T$ being $-1$. Therefore, $X_1$ is the random variable representing the result of the first coin toss, similarly for $X_2$ and $X_3$ being the result of the second and third coin toss, all independent of each other.
\[ P(X_i = 1) = P(X_i = -1) = 1/2 \quad \text{for } i=1,2,3 \] 

Now, $X_4$ should depend on $X_1,X_2,X_3$ to make sure that all four random variables are not independent (remember $X_4$ is still defined on the same $\Omega$). Extending the \textit{pairwise independence example} in Note 13, we can define $X_4 = X_1X_2X_3$, i.e. the event of the number of tails in these three coin tosses being even (e.g. 0 or 2). (You are free to try some other equivalent cases here.) We can check that 
\begin{align*}
&P(X_4 = 1) \\ 
&= P(X_1X_2X_3 = 1) \\ 
&= P(X_1=1,X_2=1,X_3=1)+P(X_1=-1,X_2=1,X_3=1)+P(X_1=1,X_2=-1,X_3=1)+P(X_1=1,X_2=1,X_3=-1) \\
&= 4 \cdot P(X_i = 1)^3  \quad\text{from the definition of} X_i, i=1,2,3 \\
&= 1/2
\end{align*}
So, $P(X_4=1)=P(X_4=-1)=1/2$, has the same distribution as the other three randome variables.

Furthermore, we need to check that any three random variables are independent and we can pick $X_1,X_2,X_4$ without loss of generality. And we can see that
\begin{align*}
P(X_1=1, X_2=1, X_4=1) &= P(X_1=1, X_2=1, X_1X_2X_3=1) \\
&= P(X_1=1, X_2=1, X_3=1) \quad \text{we can tell } X_3 \text{ if we know } X_1,X_2 \\
&= \frac{1}{8} \\
&= P(X_1=1) P(X_2=1) P(X_4=1) = (\frac{1}{2})^3
\end{align*}
...
\begin{align*}
\exx[XY] &= \sum\limits_{x \in \mathcal{X}} \sum\limits_{y \in \mathcal{Y}} xy \cdot Pr[X=x \cap Y = y] \\ 
&= \sum\limits_{x \in \mathcal{X}} \sum\limits_{y \in \mathcal{Y}} xy \cdot Pr[X=x] Pr[Y = y] 
&\text{Because $X$ and $Y$ are independent}\\ 
&= \sum\limits_{x \in \mathcal{X}} Pr[X=x] \cdot x \sum\limits_{y \in \mathcal{Y}} y \cdot Pr[Y = y]
&\text{Because $x$ does not change in the inner summation}\\ 
&= \sum\limits_{x \in \mathcal{X}} Pr[X=x] \cdot x \exx[Y] 
&\text{By definition of expectation} \\ 
&= \exx[Y] \cdot \sum\limits_{x \in \mathcal{X}} Pr[X=x] \cdot x \\
&= \exx[Y]\exx[X]
\end{align*}
}}
\fi
...
\ifsolutions{ \answer {
We start with the hint, we see that $Z = (X_1 + X_2 + \cdots X_n)$, therefore $Z^2 = (X_1 + X_2 + \cdots X_n)^2$. When we expand this out we get 
\[Z^2 = (X_1 + X_2 + \cdots X_n)\cdot(X_1 + X_2 + \cdots X_n)\] 
To see what happens here we try a smaller example, 
\[(X_1+X_2+X_3)(X_1+X_2+X_3) = (X_1^2 + X_2^2 + X_3^2 + X_1X_2 + X_1X_3 + X_2X_1 + X_2X_3 + X_3X_1 + X_3X_2)\]
In general, we have the squared term of each $X_i$ we are adding, and we have all the possible products of $X_i$ and $X_j$ where order matters and $i \neq j$. Therefore, we can generalize as 
\begin{align*}
&(X_1 + X_2 + \cdots X_n)\cdot(X_1 + X_2 + \cdots X_n)\\
& = (X_1^2 +X_2^2 + \cdots X_n^2 + X_1X_2 + X_1X_3 + \cdots X_1X_n + X_2X_1 + \cdots X_2X_n + \cdots X_nX_{n-1}) \\ 
...
\ifsolutions{ \answer {
We look at the previous expression and we realize that $Var[X_i] = \exx[X_i^2] - \exx[X_i]^2$. So, 
\begin{align*}
Var[Z] &= \sum\nolimits_{i=1}^n (\exx[X_i^2] - \exx[X_i]^2) \\
...
\ifsolutions{ \answer {
First we come up with two random variables $X$ and $Y$ such that $Var[X+Y] \neq Var[X] + Var[Y]$. We let $X$ count the number of heads after 1 flip of a fair coin and let $Y$ count the number of tails after the same 1 flip of that coin. Then $X+Y = 1$ always, and therefore the variance is 0. The variance of a constant number is 0. However, 
\begin{align*}
Var[X] &= \exx[X^2] - \exx[X]^2 \\
&= Pr[X=1]\cdot1^2 + Pr[X=0]\cdot0^2 - (Pr[X=1]\cdot1 + Pr[X=0]\cdot0)^2 \\ 
&= \frac{1}{2}\cdot1 - (\frac{1}{2}\cdot1)^2 \\
&= \frac{1}{2} - \frac{1}{4} \\
&= \frac{1}{4}
\end{align*} 

The calculation for $Var[Y]$ is the exact same. Therefore, 
\[Var[X] + Var[Y] = \frac{1}{4} + \frac{1}{4} = \frac{1}{2} \neq 0 = Var[X+Y]\] 
...
\[X: \\
Pr[X=i] = \frac{1}{5} \quad\quad \text{for } i \in \mathbb{Z}, \quad -2 \leq i \leq -2  \]
\[Y: Y=\begin{cases}
       ...
       \end{cases} 
\]
\[Pr[Y=-1]=\frac{2}{5}, Pr[Y=0]=\frac{2}{5}, Pr[Y=2]=\frac{1}{5}\]
Therefore 
\[ \exx[X] = (-2 + (-1) + 0 + 1 + 2) \cdot \frac{1}{5} = 0\] 
and 
\[ \exx[Y] = (-1 + 0) \cdot \frac{2}{5} + 2 \cdot \frac{1}{5} = 0\]
So, $\exx[X]\exx[Y] = 0$. 
...
\item We have now specified the distribution of $X$ where $X$ counts
 ...
 \ifsolutions{ \answer {
  We wish to show that $Pr[X=0] + Pr[X=1] +Pr[X=2] = 1$. 
\begin{align*}
\sum\nolimits_{k=0}^2 Pr[X=k] &= 
\binom{2}{0}(\third)^0(1-\third)^{2-0} + \binom{2}{1}(\third)^1(1-\third)^{2-1} + \binom{2}{2}(\third)^2(1-\third)^{2-2} \\ 
&= \binom{2}{0}(\third)^0(\frac{2}{3})^{2} + \binom{2}{1}(\third)^1(\frac{2}{3})^1 + \binom{2}{2}(\third)^2(\frac{2}{3})^0 \\ 
&= \binom{2}{0}({4\over9}) + \binom{2}{1}({2\over9}) + \binom{2}{2}({1\over9}) \\
&= 1\cdot{4\over9} + 2\cdot{2\over9} + 1\cdot{1\over9} \\ 
&= 1
\end{align*}
  }} 
  \fi
...
\item Use your result from part h) to show that $\sum_{k=0}^n Pr[X=k] =1$ in general, where $Pr[X=k]$ is the probability you calculated in f).
   ...
   \ifsolutions{ \answer {
 We plug in $a = (1-p)$ and $b = p.$ Then we get
\begin{align*}
\sum\nolimits_{k=0}^n Pr[X=k] 
&= \sum\nolimits_{k=0}^n \binom{n}{k} (1-p)^{n-k}p^k &\text{using our answer to part f)} \\
&= \sum\nolimits_{k=0}^n \binom{n}{k} a^{n-k}b^k &\text{plugging in the values of $a$ and $b$} \\
&= (a+b)^n &\text{by the binomial theorem in part h)} \\  
&= (1-p+p)^n = 1^n \\ 
&= 1
\end{align*}
...
\qpart
\item We can say $X \sim Binom(n,p)$ which reads as "$X$ is a binomial random variable with parameters $n$ and $p$. We lastly want to calculate $\mathbb{E}[X]$. An easy way to do this is to see $X$ as sum of independent random variables. Each $X_i$ is an indicator variable that is $1$ if the $i^{\text{th}}$ flip is heads and $0$ if the $i^{\text{th}}$ flip is tails. Argue briefly that $X = \sum\nolimits_{i=1}^nX_i$.

 \ifsolutions{ \answer {
If $X$ is a count of the total number of heads, we get this count by counting each trial being a head or not. Since each $X_i$ is 1 if and only if the $i^\text{th}$ flip is a head, the $X_i$ count each trial being a head or not. This technique of breaking a random variable into a sum of simpler random variables is a widely-used technique and you should remember this trick, especially when computing expectations.
  }} 
  \fi 
...
\item Use the linearity of expectation to calculate $\mathbb{E}[X]$.
 ...
 \ifsolutions{ \answer {
By the linearity of expectation, if $X = \sum\nolimits_{i=1}^nX_i$, then $\exx[X] = \sum\nolimits_{i=1}^n\exx[X_i]$. So now we must compute the expectation of each $X_i$. We notice that each $X_i$ is distributed identically, therefore it is sufficient to compute the expectation of only one $X_i$ and the rest will be identical. Each $X_i$ is 1 (flip heads on $i^{\text{th}}$ flip) with probability $p$ and 0 (flip tails on $i^{\text{th}}$ flip) with probability $1-p$. Therefore, 
\[ \exx[X_i] = 1\cdot p + 0\cdot(1-p) = p \]
by definition of expectation. And 
\[ \exx[X] = \sum\nolimits_{i=1}^n\exx[X_i] = \sum\nolimits_{i=1}^n p = np\]
If $X \sim Binom(n,p)$ then, $\exx[X] = np$.
  }} 
  \fi 
...
\item Looking at what this expectation is, remark on one empirical finding from a virtual lab or the lab we did during discussion that is explained by the expectation you just calculated. Congratulate yourself for finishing a problem by GSI lots-of-parts.
 ...
 \ifsolutions{ \answer {
In Discussion 8A, we saw that after $n$ flips of a $d$-sided dice, the most common value of counting the number of times we roll a 1 was close to $n/d$. We see that this is similar to flipping a biased coin with probability $p=1/d$ since we were only counting the times we roll a 1. The most common value is \underline{not} the same thing as expectation. (Some random variables never take on their expectation). However, we do know that expectation is the weighted-average. So, this finding explains why the histograms we found in Discussion 8A were symmetrical around the expectation. If it weren't symmetrical around the expectation, then the expectation wouldn't actually be the weighted average! 
...
\ifsolutions{ \answer{ 
The probability of getting $(x-1)$ non-winning lottery tickets is $(1-p)^{x-1}$. Therefore the probability of getting $(x-1)$ non-winning tickets, then a winning ticket is
	\[ f(x) = \mathbb{P}(X=x) = (1-p)^{x-1} p. \]
To show that this is a valid probability distribution, we need to make sure the sum of $f(x)$ over all possible $x$ is 1. Using the formula for an infinite geometric series,
...
\ifsolutions{ \answer{ 
This relation comes from the fact that with probability $p$, Lily will only need to buy $1$ lottery ticket. But if she is not so lucky the first time, with probability $(1-p)$ she will have to try again.  When she tries again, her expectation from that point hasn't changed, so in a sense she is just starting over. Thus we get the result from two conditional expectations: 
\[ \exx(X) = \exx(X|1st)\Pr[1st] + \exx(X|\bar{1st})\Pr[\bar{1st}] \]
where $\exx(X|1st)=1$ is the expectation of the number of lottery tickets if Lily wins the first time she buy; and $\bar{1st}$ is the complement of this event. Here, $\exx(X|\bar{1st})=1 + \exx(X)$ because Lily has to buy 1 tickt (to lose on the first time) and then start again. Therefore
\begin{align*}
\exx(X) &= 1\cdot p + (1+\exx(X))\cdot(1-p) \\
&= p+(1-p) + (1-p)\exx(X) \\
&= 1 + (1-p)\exx(X)
\end{align*}
