...
We attempted to reach a different stable pairing, but this is the exact same stable pairing. We now try to consider if it is possible to construct a pairing where everyone is paired with someone different. We prove that in all stable pairings, C must be paired with 2.
...
\item[i]If A is paired with 2, then (2,B) is a rogue couple; 
\item[ii]If A is paired with 1, then 2 is paired with B.  \end{itemize}
...
We consider by looking at case separately. 
...
We will argue that W* prefers her final M* partner to M, so that (M,W*) cannot be a rogue couple. 
