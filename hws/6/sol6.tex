...
\[ A(x) = B(x)Q(x) + R(x) \]
where $Q(x)$ and $R(x)$ can be calculated using the given function div($A(x),B(x)$). Here, $\deg{P(x)}$ denotes the degree of $P(x)$.
%\begin{verbatim}
%algorithm div (A(x), B(x))
%  //subroutine defined
%  return [Q(x), R(x)]
%\end{verbatim}
The algorithm can be extended from the original integer-based GCD as follows: 
}
\color{mydarkblue}{
\begin{verbatim}
